...
def pseudo_labeling(args, model, converter, target_data, adapting_list, round):
    ...
    # get adapt_data
    subset_data = Subset(target_data, adapting_list)
    pseudo_data = Pseudolabel_Dataset(subset_data, adapting_list)
    dataloader = get_dataloader(args, pseudo_data, args.batch_size_val, shuffle=False)
    ...
    with torch.no_grad():
        ...
        for (image_tensors, image_indexs) in tqdm(dataloader):
            ...
            if args.Prediction == "CTC":
                # No need contextual_feature here
                _, preds = model(image)
            else:
                text_for_pred = (
    ...
    # save pseudo-labels
    save_dir = "stratify/diana"
    os.makedirs(save_dir, exist_ok=True)

    with open(f"{save_dir}/pseudolabel_{round}.txt", "w") as file:
...
def self_training(args, filtered_parameters, model, criterion, converter, relative_path, \
    ...
    for iteration in tqdm(
        ...
        if args.Prediction == "CTC":
            _, preds_source = model(images_source)
            ...
            _, preds_source = model(images_source, labels_source_index[:, :-1])  # align with Attention.forward
        ...
        if args.Prediction == "CTC":
            _, preds_adapting = model(images_adapting)
            ...
            _, preds_adapting = model(images_adapting, labels_adapting_index[:, :-1])  # align with Attention.forward
...
def main(args):
    ...
    # Define relative_path for saving model and log
    relative_path = "diana_finetuning"
    ...
    try:
        # TODO: Remove map_location when running with GPUs
        pretrained = torch.load(args.saved_model, map_location=torch.device("cpu"))
    ...
    # TODO: Try to change the order of subsets
    subset_names = ["cc", "uc", "ui", "ci"]
    # subset_names = ["cc", "uc", "ci", "ui"]
    ...
    for round in range(args.num_subsets):
        ...
        # select subset
        try:
            subset_type = subset_names[round]
            adapting_list = list(
                np.load(f"stratify/{args.num_subsets}_subsets/subset_{subset_type}_idx.npy")
            )
        except FileNotFoundError:
            raise ValueError(
                f"stratify/{args.num_subsets}_subsets/subset_{subset_type}_idx.npy not found."
            )
    ...
    # save log
    print(main_log, file= open(f"log/diana/log_StrDA.txt", "w"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        # TODO: Change valid_data path to prevent error
        # "--valid_data", default="data/val/", help="path to validation dataset",
        "--valid_data", default="data/train/synth/", help="path to validation dataset",
    )
    parser.add_argument(
        "--saved_model",
        required=True, 
